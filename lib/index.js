...
import { isIntrinsicErrorType, isNamedDeclarationWithName } from 'ts-api-utils';
import typeMatchesSpecifier from 'ts-declaration-location';
import escapeRegExp from 'escape-string-regexp';
import { IndexKind, isStringLiteral, isNumericLiteral } from 'typescript';
...
function checkAssignmentExpression(node, context, rawOptions) {
    ...
    const checker = getParserServices(context).program.getTypeChecker();
    const error = isAssignableViaStrictTupleTypes(checker, getTypeOfNode(node.left, context), getTypeOfNode(node.right, context));
    if (error !== null) {
        return {
            context,
...
function checkVariableDeclaration(node, context, rawOptions) {
    ...
    const descriptors = node.declarations
        ...
        if (leftTypeAnnotation !== undefined && rightNode !== null) {
            const error = isAssignableViaStrictTupleTypes(checker, getTypeOfNode(leftTypeAnnotation, context), getTypeOfNode(rightNode, context));
            if (error !== null) {
...
function getAllSymbolDeclarations(symbol) {
...
function assignToArrayErrorIntoDescriptor(node, error) {
    return {
        node,
...
function mapGetOrInsert(map, key, orElse) {
...
function weakMapGetOrInsert(weakMap, key, orElse) {
...
const checkerToLtrToErrorCache = new WeakMap();
const setCacheAssignability = (checker, leftType, rightType, error) => {
...
const getCacheAssignability = (checker, leftType, rightType) => {
...
// const DEBUG = false;
function isAssignableViaStrictTupleTypes(checker, leftType, rightType) {
...
function assignToArrayErrorNew(ctx) {
    const { checker, tupleType, arrayType } = ctx;
    return {
        tupleType: checker.typeToString(tupleType),
        arrayType: checker.typeToString(arrayType),
    };
}
function computeIsAssignableViaStrictTupleTypes(checker, leftType, rightType) {
    ...
    for (const rightProp of rightProps) {
        ...
        const rightIsAssignableToLeftProp = (right) => {
