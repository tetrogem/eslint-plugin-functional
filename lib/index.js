...
function checkAssignmentExpression(node, context, rawOptions) {
    ...
    const error = isAssignableViaStrictTupleTypes(getTypeOfNode(node.left, context), getTypeOfNode(node.right, context), context);
    if (error !== null) {
        return {
            context,
...
function checkVariableDeclaration(node, context, rawOptions) {
    ...
    const descriptors = node.declarations
        ...
        if (leftTypeAnnotation !== undefined && rightNode !== null) {
            const error = isAssignableViaStrictTupleTypes(getTypeOfNode(leftTypeAnnotation, context), getTypeOfNode(rightNode, context), context);
            if (error !== null) {
...
const leftToRightToAssignableCache = new WeakMap();
...
function isAssignableViaStrictTupleTypes(leftType, rightType, context) {
    const checker = getParserServices(context).program.getTypeChecker();
